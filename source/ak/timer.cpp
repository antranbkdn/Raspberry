...
void timer_handler(int sig, siginfo_t *si, void *uc) {
	...
	while (timer_node_temp != NULL) {
		...
		if (timer_node_temp->data.counter <= 0) {
			...
			task_post(timer_node_temp->data.task_id, s_msg);
			AK_TIMER_DBG("[TIMER][timer_handler] task_id:%d sig:%d\n", s_msg->header->task_id, s_msg->header->sig);
		...
		if (remove_node_mask != NULL) {
			timer_service_remove_node(remove_node_mask->data.task_id, remove_node_mask->data.sig);
...
uint32_t timer_set(uint32_t task_id, uint32_t sig, uint32_t duty, timer_type_t timer_type) {
	AK_TIMER_DBG("[TIMER][timer_set] task_id:%d sig:%d duty:%d timer_type:%d\n", task_id, sig, duty, timer_type);
	timer_msg_t* timer_node_temp = timer_service.tail;

	while (timer_node_temp != NULL) {
		if (timer_node_temp->data.task_id == task_id &&
	...
	timer_new->data.task_id = task_id;
...
uint32_t timer_remove_attr(uint32_t task_id, uint32_t sig) {
	uint32_t ret;
	pthread_mutex_lock(&mt_timer_service);
	ret = timer_service_remove_node(task_id, sig);
	pthread_mutex_unlock(&mt_timer_service);
	return ret;
}
...
uint32_t timer_service_remove_node(uint32_t dst, uint32_t sig) {
	...
	while (timer_node_temp != NULL) {
		if (timer_node_temp->data.task_id == dst &&
				timer_node_temp->data.sig == sig) {
